...
@property (weak, nonatomic) SeconViewDataEntity *entity;
...
- (void)switchExpandedState:(UIButton *)button {
    [_delegate customCell:self switchExpandedStateWithIndexPath:_indexPath];
}
...
- (void)initView {
    ...
    // Content
    // UILabelpreferredMaxLayoutWidth，Multi-line must be set to this value, otherwise the system can not determine the width of the Label
    ...
    [_contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        // First with a high degree of restrictions

        _contentHeightConstraint = make.height.equalTo(@64).with.priorityHigh(); // Priority only set to High, lower than the normal height constraints, to prevent conflict
    }];
}
